...
在[上一篇文章中](/2014-05-03-nanos-note-1-bootloader/)，我们已经完成了bootloader的运行：设置内核段描述符、进入保护模式、载入ELF格式内核，并将控制交给内核代码。 但是，此时操作系统的启动过程并未完全完成，本篇文章接着bootloader完成的工作，还是以Nanos为范例，具体的分析一下操作系统内核的初始化过程（内核初始化所使用的代码可以在Nanos的github找到，见[这里](https://github.com/NJUOS/Lab1 "Nanos")）。 内核初始化主要完成了下面这些工作：
