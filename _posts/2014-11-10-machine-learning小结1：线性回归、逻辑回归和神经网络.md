...
$\\displaystyle Hypothesis: h\_\\theta(x)=\\theta^{T}x=\\theta\_0x\_0+\\cdots+\\theta\_nx\_n$

$ \\displaystyle Cost\\ function: J(\\theta)=\\frac{1}{2m}\\sum\_{i=1}^{m}(h\_\\theta(x^{(i)})-y^{(i)})^2$
...
$ \\displaystyle \\begin{aligned} &Repeat\\ \\{\\\\ &\\qquad\\theta\_j=\\theta\_j-\\alpha\\frac{\\partial}{\\partial\\theta\_j}J(\\theta)\\\\ &\\qquad(simultaneously\\ update\\ for\\ every\\ j=0,\\ldots,n)\\\\ &\\} \\end{aligned} $
...
$ \\displaystyle \\begin{aligned} h\_\\theta(x)=&g(\\theta^{T}x)\\\\ g(z)=&\\frac{1}{1+e^{-z}} \\end{aligned} $
...
$ \\displaystyle J(\\theta)=-\\frac{1}{m}\\left\[\\sum\_{i=1}^{m}y^{(i)}\\log h\_\\theta(x^{(i)})+(1-y^{(i)})\\log (1-h\_\\theta(x^{(i)}))\\right\]$
...
$ \\displaystyle \\max \_i h\_\\theta^{(i)}(x)$
...
$\\displaystyle J(\\theta)=\\frac{1}{2m}\\left\[\\sum\_{i=1}^{m}(h\_\\theta(x^{(i)})-y^{(i)})^2+\\lambda\\sum\_{j=1}^{n}\\theta\_j^2\\right\] $

逻辑回归Regularization之后的Cost function如下：

$ \\displaystyle \\begin{aligned} J(\\theta)=&-\\frac{1}{m}\\left\[\\sum\_{i=1}^{m}y^{(i)}\\log h\_\\theta(x^{(i)})+(1-y^{(i)})\\log (1-h\_\\theta(x^{(i)}))\\right\]\\\\ &+\\frac{\\lambda}{2m}\\sum\_{j=1}^{n}\\theta\_j^2 \\end{aligned} $
...
$ \\displaystyle \\begin{aligned} z^{(2)}=&\\Theta^{(1)}a^{(1)}\\\\ a^{(2)}=&g(z^{(2)})\\\\ Add\\ a\_0^{(2)}=&1\\\\ z^{(3)}=&\\Theta^{(2)}a^{(2)}\\\\ h\_\\Theta(x)=&a^{(3)}=g(z^{(3)}) \\end{aligned} $
...
$ \\displaystyle \\begin{aligned} J(\\Theta)=&-\\frac{1}{m}\\left\[\\sum\_{i=1}^{m}\\sum\_{k=1}^{K}y\_k^{(i)}\\log (h\_\\Theta(x^{(i)}))\_k+(1-y\_k^{(i)})\\log (1-(h\_\\Theta(x^{(i)}))\_k)\\right\]\\\\ &+\\frac{\\lambda}{2m}\\sum\_{l=1}^{L-1}\\sum\_{i=1}^{s\_l}\\sum\_{j=1}^{s\_{l+1}}(\\Theta\_{ji}^{(l)})^2 \\end{aligned} $
...
$ \\displaystyle \\begin{aligned} \\delta^{(4)}=&a^{(4)}-y\\\\ \\delta^{(3)}=&(\\Theta^{(3)})^{T}\\delta^{(4)}.\*g'(z^{(3)})\\\\ \\delta^{(2)}=&(\\Theta^{(2)})^{T}\\delta^{(3)}.\*g'(z^{(2)}) \\end{aligned} $
