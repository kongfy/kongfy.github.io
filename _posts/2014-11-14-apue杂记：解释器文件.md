...
如果脚本文件真的是由shell解释执行的，则不应该会产生对于_./args.py_的execve系统调用，后面的一连串系统调用都是由_/usr/bin/env_生成的寻找python位置的调用。
...
当在用户空间执行系统调用陷入内核后，通过_syscall\_table_找到并调用函数_sys\_execve_，函数简单的从用户空间拷贝了_filename_字符串，调用_do\_execve_函数： 
...
这个函数很长，你可以慢慢品读，注意中间调用了_search\_binary\_handler_函数，该函数负责寻找实际实行该文件的方式： 
